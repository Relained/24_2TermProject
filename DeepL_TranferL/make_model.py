...
_, class_count = create_mapping_and_class_count(paths["dataset_dir"])
...
# 전이 학습을 사용하여 기존 resnet50 모델에서 새로운 모델 연산
def make_model():
    # 변환 함수 실행
    convert_images(paths["dataset_dir"], processed_dataset_dir)
    # 모델 구성
    ...
    predictions = Dense(class_count, activation='softmax')(x)  # 클래스 수에 맞게 수정
    ...
    train_data = train_datagen.flow_from_directory(
        processed_dataset_dir,  # 변환된 데이터 경로
        target_size=(224, 224),
        batch_size=32,
    ...
    # 학습이 완료된 모델을 저장
    model.save("DeepL_TranferL/models/" + database["model_name"])
